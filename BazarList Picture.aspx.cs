...
public partial class BazarList_Picture : System.Web.UI.Page
{
    ...
    protected void BtnSearchByDate_Click(object sender, EventArgs e)
    {
        try
        {
            if (TextBoxStartDate.Text != "" && TextBoxEndDate.Text != "")
            {
                ...
                con.Open();
                SqlCommand cmd = new SqlCommand("select Listpicture from finalPage where hostelName=@hostelName and (date between @startDate and @endDate)", con);
